...
- `api/` - HTTP endpoints
- `core/` - конфиг и асинхронное подключение БД (SQLAlchemy)
...
- работа через SQLAlchemy AsyncSession dependency (`get_db_session`)
...
- В приложение подключается корневой роутер из `api/__init__.py`.
- Версионированные роуты лежат в `api/v1/`.
...
- Доступ к данным идет через `core/database.py` -> `repositories/notifications.py` с `AsyncSession`.
...
Для запросов используется заголовок `X-User-ID`.

## 6. Что проверить при запуске

1. Применить миграции через `uv run alembic upgrade head`.
2. Указать `DB_*` параметры в `.env`.
